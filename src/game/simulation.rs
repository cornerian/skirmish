...
pub(crate) fn enter(fighter: &mut Fighter, action: Action) {
    ...
    // `ftCo_AttackAir_EnterFromMsid` and the ordinary grounded-tilt entry
    // helpers call `ftAnim_8006EBA4` immediately after
    // `Fighter_ChangeMotionState`, advancing the selected motion to frame one
    // on its entry frame. Keep the engine's shared post-frame increment
...
pub(crate) fn advance(
    ...
    let mut idle_rng = crate::compat::math::random::HsdRng::new(state.rng_seed);
    for player in 0..2 {
        if !active[player] {
            continue;
        }
            ...
            for (index, hurtbox) in data.fighters[victim].hurtboxes.iter().enumerate() {
                ...
                if overlaps {
                    let height = data.fighters[victim]
                        .damage_poses
                        .as_ref()
                        .map_or(crate::fighter::damage::HurtHeight::Middle, |poses| {
                            poses.hurtbox_heights[index]
                        });
...
fn update_animation(
    ...
    player: usize,
    input: Controller,
    idle_rng: &mut crate::compat::math::random::HsdRng,
...
fn attack_frame<'a>(fighter: &Fighter, data: &'a FighterData) -> Result<&'a AttackFrame, Error> {
    ...
    // zero-based, so the entry clock's extra unit is not a second sample.
...
mod tests {
    ...
    #[test]
    fn ordinary_tilt_entry_reports_native_age_and_first_authored_sample() {
        let mut data = fixture();
        data.rules.countdown_frames = 0;
        ...
        assert_eq!(fighter.action, Action::AttackHi3);
        assert_eq!(fighter.action_frame, 1);
        assert_eq!(crate::fighter::tilt::sample(fighter.action, fighter.action_frame), 0);
    }
}
