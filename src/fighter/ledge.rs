...
/// Enter a ledge move through the typed registry, preserving the native
/// action when the registered move's canonical action is that same action.
/// A custom canonical action owns the transition completely; callers must not
/// apply ledge-specific setup after this function returns.
fn enter_registered(
    fighter: &mut Fighter,
    data: &FighterData,
    slot: crate::game::script::move_registry::MoveSlot,
    native: Action,
) -> crate::game::script::move_selection::NativeMoveSelection {
    let selected = crate::game::script::move_selection::select_native_move_variant(
        fighter,
        data,
        crate::game::script::move_registry::MoveGroup::Ledge,
        slot,
        native,
        native,
    );
    // A custom destination leaves the native Cliff* family.  Release the
    // supporting line so validation and collision cannot continue treating a
    // custom action as a native ledge animation.
    if matches!(
        selected,
    ...
    match fighter.action {
        Action::CliffCatch if fighter.action_frame as usize >= parameters.catch.frames.len() => {
            let _ = enter_registered(
                fighter,
                data,
                crate::game::script::move_registry::MoveSlot::Wait,
                Action::CliffWait,
            );
...
pub(crate) fn update_actions(
    ...
    let action = if pressed & crate::game::BUTTON_A != 0 {
        Some(Action::CliffAttack)
    } else if pressed & (crate::game::BUTTON_L | crate::game::BUTTON_R) != 0 {
        Some(Action::CliffEscape)
    } else if pressed & (crate::game::BUTTON_X | crate::game::BUTTON_Y) != 0 {
        ...
        if !main_active && !c_active {
            ...
            .map(|option| match option {
                StickOption::Climb => Action::CliffClimb,
                StickOption::Drop => Action::Fall,
            })
        }
    };
    if let Some(Action::Fall) = action {
        ...
        let slot = match action {
            Action::CliffClimb => crate::game::script::move_registry::MoveSlot::Getup,
            Action::CliffEscape => crate::game::script::move_registry::MoveSlot::Roll,
            Action::CliffJump => crate::game::script::move_registry::MoveSlot::Jump,
            Action::CliffAttack => crate::game::script::move_registry::MoveSlot::Attack,
            _ => unreachable!("ledge option produced an unsupported action"),
        };
        let _ = enter_registered(fighter, data, slot, action);
    }
    true
}
...
pub(crate) fn scan(
    ...
    for (player, controller) in controllers.into_iter().enumerate() {
        ...
        if fighter.ledge.cooldown > 0
            ...
            || !catchable(fighter.action, &data.fighters[player])
...
fn finish_on_floor(fighter: &mut Fighter, geometry: &StageGeometry) -> Result<(), Error> {
    ...
    fighter.ledge.line = None;
    fighter.ledge.side = None;
    crate::fighter::locomotion::landed(fighter);
...
fn catchable(action: Action, data: &crate::game::data::FighterData) -> bool {
    matches!(
        action,
        Action::Fall
            | Action::DamageFall
            ...
            | Action::AttackAirHi
            | Action::AttackAirLw
    ) || crate::fighter::specials::ledge_catchable(action, data)
}
...
/// box might otherwise contain; `fighter::ledge::scan` already knows it is
