...
// are not modeled.
use crate::fighter::tilt::{Chain, GroundFrameFlags};
use crate::game::{
...
pub(crate) fn try_taunt(
    ...
    if !fighter.grounded || !pressed(pressed_buttons, crate::game::BUTTON_DPAD_UP) {
        return Ok(false);
    }
    let action = match select_side(fighter.facing, taunt.left.is_some()) {
        Side::Left => Action::AppealSL,
        Side::Right => Action::AppealSR,
    };
    match crate::game::script::move_selection::select_native_move_variant(
        fighter,
        data,
        crate::game::script::move_registry::MoveGroup::Taunt,
        crate::game::script::move_registry::MoveSlot::Taunt,
        Action::AppealSR,
        action,
    ) {
        ...
        | crate::game::script::move_selection::NativeMoveSelection::Unbound(destination)
            if matches!(destination, Action::AppealSR | Action::AppealSL) =>
        {
            fighter.body_state = sampled_body_state(fighter, data)?;
        }
