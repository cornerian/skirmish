...
pub fn decay_window(window: &mut f32) {
    if *window > 0.0 {
        *window -= 1.0;
    }
}
...
fn a_released(fighter: &Fighter, input: Controller) -> bool {
    crate::fighter::smash::logical_a(fighter.previous_input.buttons)
        && !crate::fighter::smash::logical_a(input.buttons)
}
...
fn start_registered(
    ...
    matches!(
        crate::game::script::move_selection::select_native_move(
            fighter,
            data,
...
fn start_first(fighter: &mut Fighter, data: &FighterData) {
    let Some(parameters) = data.jab_combo.as_ref() else {
        start_registered(
            fighter,
            data,
            crate::game::script::move_registry::MoveSlot::Jab,
            Action::Jab,
        );
        return;
    };
...
/// `ftCo_800D6B00`.
fn start_rapid(fighter: &mut Fighter, data: &FighterData, parameters: &Parameters) {
...
pub(crate) fn press(fighter: &mut Fighter, data: &FighterData) -> bool {
    let Some(parameters) = data.jab_combo.as_ref() else {
        crate::game::simulation::enter(fighter, Action::Jab);
        return true;
    };
    match wait_press(fighter.jab.window, fighter.jab.follow_up, fighter.jab.last) {
        ...
        None => {
            decay_window(&mut fighter.jab.window);
            return false;
        }
    }
    true
    ...
    match fighter.action {
        Action::Jab | Action::Attack12 | Action::Attack13 => {
            if rapid_count(
                ...
                start_rapid(fighter, data, parameters);
                return true;
            }
            let follow_up = fighter.jab.follow_up;
            let (window, buffered) = (&mut fighter.jab.window, &mut fighter.jab.buffered);
            match fighter.action {
                Action::Jab if buffer_follow_up(window, pressed, buffered, follow_up) => {
                    start_second(fighter, parameters);
                    true
                }
                Action::Attack12 if buffer_follow_up(window, pressed, buffered, follow_up) => {
...
pub(crate) fn update_animation(fighter: &mut Fighter, data: &FighterData) -> Result<(), Error> {
    ...
    if fighter.action == Action::Attack100Loop {
        ...
        if fighter.action == Action::Attack100Start {
            crate::game::simulation::enter(fighter, Action::Attack100Loop);
    ...
    if flags.loop_check
        ...
        && loop_check(fighter.jab.loop_started, &mut fighter.jab.loop_input)
    {
        crate::game::simulation::enter(fighter, Action::Attack100End);
    }
    Ok(())
}
