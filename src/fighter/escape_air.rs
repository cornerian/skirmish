// Air-dodge arithmetic and motion callbacks from `ftCo_EscapeAir.c`, plus the
// FallSpecial landing predicate from `ftCo_FallSpecial.c`. Item-throw restore
// copying, the `x334` item timer, tether interrupts, and parasol remain out.

use crate::fighter::aerial as landing_math;
use crate::game::{
...
pub(crate) fn platforms_land(fighter: &Fighter, rules: Option<&Rules>, input: Controller) -> bool {
    ...
    fighter.action != Action::FallSpecial
        || self::platform_landing(true, true, input.stick[1], rules.platform_stick_threshold)
}
...
fn eligible(fighter: &Fighter) -> bool {
    !fighter.grounded
        ...
        ) || crate::fighter::damage::damage_air_interruptible(fighter)
            || crate::fighter::damage::wall_tech_interruptible(fighter))
}
...
pub(crate) fn try_air_dodge(
    ...
    if !eligible(fighter)
        || input.buttons
            & !fighter.previous_input.buttons
            & (crate::game::BUTTON_L | crate::game::BUTTON_R)
            == 0
    {
        return Ok(false);
    }
    fighter.velocity = self::launch_velocity(input.stick, rules.deadzone, rules.force);
    let selected = crate::game::script::move_selection::select_native_move(
        fighter,
        data,
        crate::game::script::move_registry::MoveGroup::Defense,
        crate::game::script::move_registry::MoveSlot::AirDodge,
        Action::EscapeAir,
    );
    if matches!(
        selected,
    ...
    // `ftCo_Turn_Enter` make; modeled at the source like `fighter::locomotion::
    ...
    match fighter.action {
        Action::EscapeAir => {
            ...
            if sample >= parameters.frames.len() {
                ...
                crate::game::simulation::enter(fighter, Action::FallSpecial);
...
pub(crate) fn land(
    ...
    crate::game::simulation::enter(fighter, Action::LandingFallSpecial);
