...
Streaming machinery for validating a future replay adapter against a headless
...
The tests execute explicitly scripted sequences of existing `melee-physics`
...
cargo test --locked -p skirmish-replay
cargo clippy --locked -p skirmish-replay --all-targets -- -D warnings
```
