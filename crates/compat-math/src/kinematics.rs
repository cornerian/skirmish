...
/// the ground-speed-retention fraction and the active floor-friction
/// multiplier. The multiplier defaults to `1.0` for callers that use the
/// ordinary-terrain profile.
pub fn entry_ground_velocity(ground_velocity: f32, retention: f32) -> f32 {
    entry_ground_velocity_with_friction(ground_velocity, retention, 1.0)
}

/// Full `doEnter` arithmetic when the caller has the floor-friction lookup.
pub fn entry_ground_velocity_with_friction(
    ground_velocity: f32,
    retention: f32,
    friction_multiplier: f32,
) -> f32 {
    ground_velocity + (-(ground_velocity * (1.0 - retention)) * friction_multiplier)
}
...
mod tests {
    ...
    #[test]
    fn entry_ground_velocity_applies_source_friction_multiplier() {
        assert_eq!(entry_ground_velocity_with_friction(10.0, 0.5, 0.5), 7.5);
        assert_eq!(entry_ground_velocity_with_friction(-10.0, 0.5, 2.0), -0.0);
    }
