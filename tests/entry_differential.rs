...
use skirmish::game::flow::entry::{amplitude, end_progress, start_progress};
...
proptest! {
    ...
    /// `game::flow::entry::enter_start`'s own assumption).
    #[test]
...
proptest! {
    ...
    /// `game::flow::entry::update_animation`'s `Action::Entry` arm calling
