/* Host adapter for the complete down-tilt input predicate, its entry and
 * the buffered repeat check. The Game & Watch override is disabled. */
#include <math.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
static void ftGw_AttackLw3_Enter(Fighter_GObj* gobj) { (void) gobj; captured_motion = -1; }
static void callUnk(Fighter_GObj* gobj) { (void) gobj; }
#include "attack_lw3_original.inc"

int oracle_down_tilt(const float* stick, float threshold, float angle_limit,
                     uint32_t pressed, int* motion, int* flags)
{
    common = (ftCommonData) { .x20_radians = angle_limit, .xB0 = threshold };
    p_ftCommonData = &common;
    Fighter fighter = { .input = { .lstick = {{ stick[0], stick[1] }}, .pressed_buttons = pressed } };
    Fighter_GObj gobj = { &fighter };
    captured_motion = 0;
    captured_flags = 0;
    int result = ftCo_AttackLw3_CheckInput(&gobj);
    *motion = captured_motion;
    *flags = captured_flags;
    return result;
}

/* Complete checkPadA: returns the result, the updated buffer and the motion. */
int oracle_down_tilt_repeat(uint32_t pressed, int repeat_ready, int buffered,
                            int* buffer_after, int* motion)
{
    Fighter fighter = { .input = { .pressed_buttons = pressed },
                        .cmd_vars = { repeat_ready },
                        .mv = { .co = { .attacklw3 = { buffered } } } };
    Fighter_GObj gobj = { &fighter };
    captured_motion = 0;
    int result = checkPadA(&gobj);
    *buffer_after = fighter.mv.co.attacklw3.x0;
    *motion = captured_motion;
    return result;
}
