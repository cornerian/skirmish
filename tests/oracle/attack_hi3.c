/* Host adapter for the complete up-tilt input predicate. */
#include <math.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
#include "attack_hi3_original.inc"

int oracle_up_tilt(const float* stick, float threshold, float angle_limit,
                   uint32_t pressed, int* motion)
{
    common = (ftCommonData) { .x20_radians = angle_limit,
                              .attackhi3_stick_threshold_y = threshold };
    p_ftCommonData = &common;
    Fighter fighter = { .input = { .lstick = {{ stick[0], stick[1] }}, .pressed_buttons = pressed } };
    Fighter_GObj gobj = { &fighter };
    captured_motion = 0;
    int result = ftCo_AttackHi3_CheckInput(&gobj);
