 ...
 * silently assumed):
 *   - `rotate` is stored as `Vec3`, not the real `Quaternion` (a superset
 *     struct). `HSD_JObjMakeMatrix`'s own Euler branch already reinterprets
 *     `&jobj->rotate` as `Vec3*` via an explicit cast; the caller here
 *     never sets JOBJ_USE_QUATERNION (0x20000), so the wider real field
 *     and the narrower stub are never distinguished by the code under
 *     test. `HSD_MtxSRTQuat` itself is an abort-on-call stub for the same
 *     reason: unreachable with these flags, but the symbol must exist for
 *     the pinned function's own quaternion branch to link.
 ...
 *   - `PSMTXConcat` is real GameCube paired-single assembly; `C_MTXConcat`
 *     (already pinned in `sdk_mtx.c`, `docs/math.md`'s FMA audit) is its
 *     scalar C equivalent and is substituted with a macro, the same
 *     substitution `bones.c`'s own callers already rely on.
 *   - `HSD_MtxSRT` (`sysdolphin/baselib/mtx.c:362-410`) is re-extracted
 *     under its own adapter name (`bones_pose_mtx`, `adapters.json`,
 *     `bones_pose_mtx.functions.json`) instead of reusing `bones.c`'s own
 *     copy: that copy deliberately keeps host libm `sinf`/`cosf`
 ...
 * comment): `MTXMultVec` is the aobj-translation-override path's own
 * paired-single primitive (distinct from the already-pinned scalar
 * `C_MTXMultVec` in `sdk_mtxvec.c`, which a *different* HSD_JObjMakeMatrix
 * caller uses for ordinary point transforms), and `HSD_MtxSRTQuat` is the
 * quaternion-rotation branch. Every joint here has `aobj == NULL` and
 * never sets JOBJ_USE_QUATERNION (0x20000), so neither body ever runs;
 * both symbols must still exist for `HSD_JObjMakeMatrix`'s own compiled
 * body to link, and abort loudly instead of silently doing the wrong
 * thing if that invariant is ever violated. */
...
static void HSD_MtxSRTQuat(Mtx m, Vec3* scale, void* rotate, Vec3* translate,
                            Vec3* parent_scale)
{
    (void) m;
    (void) scale;
...
void oracle_bones_pose(const int32_t* parent, const int32_t* classical_scale,
                        ...
                        const float* translation, int32_t count,
                        float* out_matrices)
{
