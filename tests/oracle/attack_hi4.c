/* Host adapter for the complete up-smash input predicate and its windowless KneeBend variant.
...
static bool ftCo_800DF30C(Fighter* fp) { return ftCo_800DF2D8(fp); }
...
static void ftNs_AttackHi4_Enter(Fighter_GObj* gobj) { (void) gobj; captured_motion = -1; }
void doEnter(Fighter_GObj* gobj);

#include "attack_hi4_original.inc"
...
static Fighter setup(const float* values, const float* rules, uint32_t pressed,
                     uint32_t tilt_y_age)
{
    common = (ftCommonData) { .xCC = rules[0], .xD0 = rules[1] };
...
int oracle_up_smash(const float* values, const float* rules, uint32_t pressed,
                     uint32_t tilt_y_age, int* motion)
{
    Fighter fighter = setup(values, rules, pressed, tilt_y_age);
    Fighter_GObj gobj = { &fighter };
    captured_motion = 0;
    int result = ftCo_AttackHi4_CheckInput(&gobj);
    *motion = captured_motion;
    return result;
}

int oracle_up_smash_no_window(const float* values, const float* rules, uint32_t pressed,
                              uint32_t tilt_y_age, int* motion)
{
    Fighter fighter = setup(values, rules, pressed, tilt_y_age);
    Fighter_GObj gobj = { &fighter };
    captured_motion = 0;
    int result = ftCo_AttackHi4_CheckInputNoD0(&gobj);
    *motion = captured_motion;
    return result;
}
