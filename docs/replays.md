...
The named **`fighter-post-v10`** policy compares these post-frame fields for each
mapped actor:
...
| selected `state_flags` | Reflector, protection, fast-fall, hitlag, active shield, hitstun, powershield, dead and sleep/inactive bits | Exact bits |
...
geometry, the shield-touch, offscreen and follower state-flag bits, items and
stage state are not compared. Reflect and powershield bits follow the independent
GuardReflect timers, including their active-at-zero boundary. The selected dead bit
