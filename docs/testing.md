...
| `taunt` | Entry from Wait, Walk, Squat, SquatWait, Turn (post-turn facing), Run, Landing's interruptible frame, an interruptible smash pose and Ottotto, and from Dash falling through into the `x54` friction tail while the same shared check from Run does not; facing left picking AppealSL when supplied and AppealSR otherwise; every other D-pad bit inert; the taunt's own interruptible chain admitting catch, smash, tilt, jab, the Wait-chain spot dodge and shield while excluding jump, a dash-magnitude stick alone and a downward stick without the shoulder; exact sampled root motion ending into Wait; clamping at a floor end; the Wait-chain spot dodge dodging immediately from Wait and an interruptible AppealS frame only (not Walk, not a stale stick, not a held C-stick alone); checkpoint replay and invalid resources |
| `fox_side_special` | Grounded and aerial entry (ground-speed retention, the `x28` entry division, the `x688` fresh-press-plus-stick age gate, jumps left untouched or restored), the strict turn-around boundary, a vertical stick suppressing the aerial Hi (unmodeled)/Lw (`fox_down_special`, modeled since the down-special batch) branches, the ground and air TransN-driven dash including the no-sample friction fallback, B-press shortening into the End phase on the ground and in the air, a mid-Start ground<->air conversion preserving both the frame and the gravity delay (which ticks down on the ground too), the End phase's dedicated speeds and frictions, the FallSpecial exit with the scaled freefall mobility and every jump restored, Slippi 347..352 ids, checkpoint replay, invalid resources and `None` keeping B+side inert. No C-oracle differential harness or Peppi replay fixtures exist for this move yet; see `docs/fox-side-special.md`'s closing section |
| `fox_down_special` | The five-phase Start/Loop/Turn/Hit/End state machine: the strict-ground/inclusive-air entry boundary and momentum division, holding B keeping Loop active past the release lag, a release only exiting once the lag has also elapsed, jump-cancel from the ground loop and aerial jump from the air loop, an immediate facing flip on the first Turn step with the countdown returning to Loop, a platform drop keeping the move and setting `reflecting`, a ground<->air conversion preserving the frame and release state, the Hit phase's own wiring despite being unreachable in play, Slippi 360..369 ids and names, checkpoint replay, invalid resources and `None` keeping B+down inert. C-oracle differential coverage and two self-recorded replay regressions (ground and air) exist for this move; see `docs/fox-down-special.md`'s "C-oracle coverage" section |
...
`taunt_differential` snapshots `ftCo_AppealS.c` for the first time
(`tests/oracle/original/appeal.c`, `ftCo_800DE9B8`/`ftCo_800DE9D8`/
`ftCo_800DEAE8`/`ftCo_800DEBD0`/`ftCo_AppealS_IASA`; `ftCo_800DEA28` is
hand-written in the adapter instead, since it is outside this batch's
selection, with every per-character side effect stubbed and `kind` fixed
at 0 so it always takes the real function's own `default` branch),
comparing entry selection (`fighter::taunt::select_side`/`pressed`) and the
complete `ftCo_AppealS_IASA` consultation order (a pure Rust mirror of its
14-check RETURN_IF chain) against the C oracle over 512 proptest cases
each, plus exact boundaries (no press, every facing/availability
combination, NaN facing, the chain locked entirely, each of the 14 checks
as the sole winner, none winning, every check winning). It also extends
the pinned `escape.c` snapshot with `ftCo_80099794` (the Wait-chain-only
spot dodge, distinct from `ftCo_8009980C`'s existing C-stick-inclusive
predicate: a held logical shoulder is required, and the C-stick is not an
alternative), comparing `held && fighter::escape::main_stick_spot_dodge`
against the extended oracle over 512 proptest cases plus the same
threshold/window boundaries `escape_differential` already uses.
