...
| Super Jump Punch (347/348) | Both entries clear command slot 0 and `throw_flags`; aerial entry also resets velocity through source attributes. Ground animation ends through the special landing/fall handler; aerial entry shares that terminal callback. | Command slot 0 reset and ground/air action pairing are represented. | Entry and transition policy are covered; velocity and landing-lag attributes remain host-owned. |
